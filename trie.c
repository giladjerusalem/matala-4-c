...
void printN(node* root, char* word, int wordSize){
    ...
    for (int i = 0; i < 26; i++){   //run each child
        if(root->children[i] != NULL){
            printN(root->children[i], newWord, wordSize+1);//call and print each child in recursion
        }
    }
...
void printR(node* root, char* word, int wordSize){
    ...
    for (int i = 25; i >= 0; i--){   //run each child
        if(root->children[i] != NULL){
            printR(root->children[i], newWord, wordSize+1);//call and print each child in recursion
        }
    }
